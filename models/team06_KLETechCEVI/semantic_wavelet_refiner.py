...
class BiSeNetParser(nn.Module):
    SKIN_CLASSES  = [1, 2, 3, 10]
    EYE_CLASSES   = [4, 5, 6]
    MOUTH_CLASSES = [11, 12, 13]
    HAIR_CLASSES  = [17, 18]
    BG_CLASSES    = [0, 7, 8, 9, 14, 15, 16]
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            out = self.net(x_norm)[0]
            parsed = out.argmax(dim=1)
...
class SemanticWaveletRefiner(nn.Module):
    def __init__(self, bisenet_path):
        ...
        self.cnn_skin  = RegionCNN(in_channels=9, hidden=32)
        self.cnn_hair  = RegionCNN(in_channels=9, hidden=32)
        self.cnn_eye   = RegionCNN(in_channels=9, hidden=32)
        self.cnn_mouth = RegionCNN(in_channels=9, hidden=32)
        self.cnn_base  = RegionCNN(in_channels=9, hidden=32)
        ...
        print(f"SemanticWaveletRefiner initialized — trainable params: {trainable:,}")
    ...
    def forward(self, stage2_out):
        ...
        def ds(m): return F.interpolate(m, size=(Hh, Wh), mode='bilinear', align_corners=False)
        m_skin  = ds(masks['skin'])
    ...
    def compute_loss(self, corrected, hq, stage2_out):
        ...
        pred_r = F.interpolate(corrected * 2 - 1, (160, 160), mode='bilinear', align_corners=False)
        hq_r   = F.interpolate(hq * 2 - 1,        (160, 160), mode='bilinear', align_corners=False)
...
class Stage2Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hq = self.transform(Image.open(hq_path).convert('RGB').resize((512, 512), Image.BICUBIC))
        return s2, hq
...
def main():
    ...
    parser.add_argument('--bisenet_path', default='./model_zoo/team06_KLETechCEVI/79999_iter.pth')
    parser.add_argument('--stage2_dir',  default='./stage2_outputs')
    parser.add_argument('--hq_dir',      default='./ffhq')
    ...
    if args.mode == 'train':
        ...
        for epoch in range(args.epochs):
            ...
            torch.save(model.state_dict(), os.path.join(args.save_dir, f"semantic_epoch{epoch+1}.pth"))
            if loss < best_loss:
                best_loss = loss
                torch.save(model.state_dict(), args.checkpoint)
                print("   New best!")

        print(f"\nDone. Best loss={best_loss:.4f}. Saved to {args.checkpoint}")
        return
    ...
    images = sorted(f for f in os.listdir(args.input_dir) if f.lower().endswith('.png'))
    for name in tqdm(images, desc="Processing"):
        img = Image.open(os.path.join(args.input_dir, name)).convert('RGB')
        t   = transform(img).unsqueeze(0).to(device)
        with torch.no_grad():
            out = model(t)
        T.ToPILImage()(out.squeeze(0).cpu()).save(os.path.join(args.output_dir, name))
