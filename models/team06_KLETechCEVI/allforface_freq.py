...
# Add the AllForFace directory to path so we can import modules
sys.path.append('/NTIRE2026/runs/C15_RWFaceRestoration/AllForFace')
...
# ── Wavelet Frequency Refiner (your novelty) ──────────────────────────────────
# Add the current directory to path as well
sys.path.append('/NTIRE2026/runs/C15_RWFaceRestoration/AllForFace')
from semantic_wavelet_refiner import SemanticWaveletRefiner
# ───────────────────────────────────────────────────────────────── ────────────


def main(model_dir, input_path, output_path, device,
         wavelet_checkpoint='./checkpoints_freq/wavelet_best.pth'):
    ...
    # Fix the config path - remove "_zoo" replacement if needed
    config_path = os.path.join(os.path.dirname(__file__), 'models/team01_AllForFace/ldm/cldm.yaml')
    if not os.path.exists(config_path):
        # Try alternative path
        config_path = os.path.join(model_dir.replace("_zoo", "s"), "ldm/cldm.yaml")
    ...
    sd_weight = load_model_from_url(os.path.join(model_dir, sd_path))
    control_net.load_pretrained_sd(sd_weight)
    
    control_weight = load_model_from_url(os.path.join(model_dir, control_path))
    ...
    # ── YOUR MODULE: Wavelet Frequency Refiner ────────────────────────────────
    wavelet_refiner = SemanticWaveletRefiner(
        bisenet_path='/NTIRE2026/runs/C15_RWFaceRestoration/AllforFace/face-parsing.PyTorch/res/cp/79999_iter.pth'
    ).to(device)
    if os.path.exists(wavelet_checkpoint):
        ...
        print(f"⚠️  Wavelet checkpoint not found at {wavelet_checkpoint} — using untrained")
    ...
    for i, img_path in enumerate(input_img_list):
        ...
        with torch.no_grad():
            ...
            # ── Wavelet Frequency Refiner (between Stage 2 and Stage 3) ──────
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) < 5:
        print("Usage: python allforface_freq.py <model_dir> <input_path> <output_path> <device> [wavelet_checkpoint]")
        sys.exit(1)
    ...
    device = sys.argv[4]
    wavelet_checkpoint = sys.argv[5] if len(sys.argv) > 5 else './wavelet_refiner.pth'
    
    main(model_dir, input_path, output_path, device, wavelet_checkpoint)